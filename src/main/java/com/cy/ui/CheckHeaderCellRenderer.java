...
import com.cy.bean.AndroidViewsTableModel;
...
public class CheckHeaderCellRenderer implements TableCellRenderer {
    private AndroidViewsTableModel mTableModel;
    ...
    public CheckHeaderCellRenderer(JXTable table) {
        this.mTableModel = (AndroidViewsTableModel) table.getModel();
        this.mTableHeader = table.getTableHeader();
        mChoiceSelectBox = new JCheckBox(mTableModel.getColumnName(AndroidViewsTableModel.CHOICE_BOX_INDEX));
        mClickBox = new JCheckBox(mTableModel.getColumnName(AndroidViewsTableModel.CLICK_BOX_INDEX));
        ...
        mTableHeader.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() > 0) {
                    ...
                    if (selectColumn == AndroidViewsTableModel.CHOICE_BOX_INDEX) {
                    ...
                    } else if (selectColumn == AndroidViewsTableModel.CLICK_BOX_INDEX) {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value,
        ...
        if (column == AndroidViewsTableModel.CHOICE_BOX_INDEX) {
            component = mChoiceSelectBox;
        }
        if (column == AndroidViewsTableModel.CLICK_BOX_INDEX) {
            component = mClickBox;
        }
